...
@pytest.mark.parametrize(
    "scenario", ["success", "blocked", "error", "oversized", "strict_success", "strict_oversized"]
)
def test_parent_bounds_whole_worker_and_isolates_secrets(tmp_path, monkeypatch, scenario):
    ...
    calls = []
    truncate = not scenario.startswith("strict_")
    program = """
import os,sys,time
from walkietalk import grok_tts
...
""".replace("SCENARIO", repr(scenario)).replace("TRUNCATE", repr(truncate))
    ...
    if scenario in {"success", "strict_success"}:
        speech = grok_tts.GrokTts(config).synthesize("Hello.", truncate=truncate)
        ...
        message = {"blocked": "timed out", "error": "HTTP 403", "strict_oversized": "maximum"}[
            scenario
        ]
        with pytest.raises(WalkietalkError, match=message):
            grok_tts.GrokTts(config).synthesize("Hello.", truncate=truncate)
