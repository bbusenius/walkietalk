...
def test_missing_realtime_section_uses_defaults(tmp_path, config_data):
    del config_data["agent"]["realtime"]
    config = load_config(write_config(tmp_path, config_data))
    assert config.agent_realtime_model == Config().agent_realtime_model
...
def test_response_start_pings_only_fails_fast(monkeypatch, supervised, ping_delay):
    ...
    assert types[types.index("input_audio_buffer.commit") + 1] == "response.create"
    if supervised:
        assert types[-1] == "response.cancel"
...
def test_supervised_keys_on_audible_energy_unkeys_on_audio_done():
    ...
    assert played == [loud_a]  # Playback starts before the next delta or done event.
